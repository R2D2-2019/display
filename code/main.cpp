...
int main() {
    ...
    color_display.clear();
    // color_display.flush();
    color_display.set_cursor_color(
        static_cast<uint8_t>(r2d2::claimed_display_cursor::OPEN_CURSOR),
        hwlib::red);

    // Set position of a claimed cursor to x = 15, y = 50
    color_display.set_cursor_position(
        static_cast<uint8_t>(r2d2::claimed_display_cursor::OPEN_CURSOR), 15,
        50);
    color_display.set_character(0, 0, 'H', color_display.color_to_pixel(hwlib::red));
    color_display.flush();
    //for (;;) {
        // color_display.clear();
        // display.set_cursor_position(
        //     static_cast<uint8_t>(claimed_display_cursor::YOUR_CURSOR), 15,
        //     50);
        // display.set_character(0, 0, 'H', display.color_to_pixel(hwlib::red));
        // color_display.flush();
        // hwlib::wait_ms(1000);
